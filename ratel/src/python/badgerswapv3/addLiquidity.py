...
def addLiquidity(appContract, token0, token1, priceLower, priceUpper, maxAmtA, maxAmtB):
    ...
    idxPriceLower, idxPriceUpper, idxMaxAmtA, idxMaxAmtB = reserveInput(web3, appContract, 4, account)
    maskPriceLower, maskPriceUpper, maskMaxAmtA, maskMaxAmtB = asyncio.run(get_inputmasks(f'{idxPriceLower},{idxPriceUpper},{idxMaxAmtA},{idxMaxAmtB}'))
    ...
    tx_hash = appContract.functions.addLiquidity(token0, token1, idxPriceLower, maskedPriceLower, idxPriceUpper, maskedPriceUpper, idxMaxAmtA, maskedMaxAmtA, idxMaxAmtB, maskedMaxAmtB).transact()
